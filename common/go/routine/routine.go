...
func NewPermanentError(message string, args ...any) *PermanentError {
	return &PermanentError{Err: errors.Errorf(message, args...)}
}
...
func (r *Routine) WithTicker(seconds int) *Routine {
...
func (r *Routine) WithTickerMs(ms int) *Routine {
	return r.withTicker(time.Duration(ms) * time.Millisecond)
}
		...
		for {
			...
			if err != nil {
				if errors.Is(err, &PermanentError{}) {
					log.Panicf("routine %s: %v", r.name, err)
				}
				log.Errorf("routine %s: %v", r.name, err)
