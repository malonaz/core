...
func GetMigrations(fileLoader FileLoader, migrationDirectory string) ([]*Migration, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not parse migrations file")
	}
	...
	for _, migration := range migrationsFile.Migrations {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "could not open migration %s/%s", migrationDirectory, migration.Filename)
		}
