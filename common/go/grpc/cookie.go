...
import (
	...
	"common/go/grpc/types"
)

// /////////////////////////////////////////////////////////////////////////////////////////
// /////////////////////////////// COOKIE CONVERSION METHODS ///////////////////////////////
// /////////////////////////////////////////////////////////////////////////////////////////
func cookieToProto(httpCookie *http.Cookie) *types.HttpCookie {
	return &types.HttpCookie{
...
func cookieFromProto(httpCookie *types.HttpCookie) *http.Cookie {
	return &http.Cookie{
		Name:  httpCookie.Name,
		Value: httpCookie.Value,
...
func (GatewayCookie) forwardOutOption(ctx context.Context, w http.ResponseWriter, _ proto.Message) error {
	...
	for _, value := range md.HeaderMD.Get(grpcGatewayCookieMetadataKey) {
		httpCookie := &types.HttpCookie{}
		if err := proto.Unmarshal([]byte(value), httpCookie); err != nil {
			return errors.Wrap(err, "unmarshaling cookie from metadata")
		}
...
func (GatewayCookie) SetHTTPCookies(ctx context.Context, httpCookies ...*types.HttpCookie) error {
	md := metadata.MD{}
	for _, httpCookie := range httpCookies {
		bytes, err := proto.Marshal(httpCookie)
		if err != nil {
			return errors.Wrap(err, "marshaling http cookie")
		}
		md.Append(grpcGatewayCookieMetadataKey, string(bytes))
	}
...
func (GatewayCookie) GetHTTPCookies(ctx context.Context) ([]*types.HttpCookie, error) {
	md, ok := metadata.FromIncomingContext(ctx)
	if !ok {
		return nil, nil
	}
	httpCookies := []*types.HttpCookie{}
	values := md.Get(grpcGatewayCookieMetadataKey)
	for _, value := range values {
		httpCookie := &types.HttpCookie{}
		if err := proto.Unmarshal([]byte(value), httpCookie); err != nil {
			return nil, errors.Wrap(err, "unmarshaling cookie from metadata")
		}
...
func (gc GatewayCookie) GetHTTPCookie(ctx context.Context, name string) (*types.HttpCookie, error) {
...
func (WebCookie) SetHTTPCookies(ctx context.Context, httpCookies ...*types.HttpCookie) error {
	md := metadata.MD{}
	for _, httpCookie := range httpCookies {
...
// GetHTTPCookies retrieves any http cookies from a context.
func (WebCookie) GetHTTPCookies(ctx context.Context) []*types.HttpCookie {
	md, ok := metadata.FromIncomingContext(ctx)
	if !ok {
		return nil
	}
...
// GetHTTPCookie retrieves the http cookie with the given name from the context.
func (WebCookie) GetHTTPCookie(ctx context.Context, name string) *types.HttpCookie {
	md, ok := metadata.FromIncomingContext(ctx)
	if !ok {
		return nil
	}
