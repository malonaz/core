...
func NewServer(config Config) (*Server, error) {
	...
	initJob, err := binary.New("postgres-initdb", binaryPath("initdb"), "--nosync", "-D", config.DataDirectory, "--auth", "trust", "-U", config.User)
	if err != nil {
		return nil, errors.Wrap(err, "could not instantiate init job")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not instantiate start job")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not instantiate stop job")
	}
...
func (s *Server) Run() error {
	if err := s.initJob.RunAsJob(); err != nil {
		return errors.Wrap(err, "could not run init job")
	}
	...
	if err := s.startJob.RunAsJob(); err != nil {
		return errors.Wrap(err, "could not run start job")
	}
	return nil
}
...
func (s *Server) Shutdown() error {
	...
	if err := s.stopJob.RunAsJob(); err != nil {
		return errors.Wrap(err, "could not run stop job")
	}
	s.stopJob.Exit()
	s.startJob.Exit()
	s.initJob.Exit()
	if err := os.RemoveAll(s.config.DataDirectory); err != nil {
		return errors.Wrap(err, "could not delete Postgresql data directory")
	}
	return nil
}
...
func (s *Server) createSocketDirectory() error {
	if err := os.MkdirAll(s.config.DataDirectory+socketFilepath, os.ModeDir|os.ModePerm); err != nil {
		return errors.Wrap(err, "could not create socket directory")
	}
	return nil
}
...
func (s *Server) writeConfigToDisk() error {
	...
	defer f.Close()
	if err != nil {
		return errors.Wrap(err, "could not create postgresql.conffile")
	}
	writer := bufio.NewWriter(f)
	for key, value := range m {
		if _, err := fmt.Fprintf(writer, "%s = %s\n", key, value); err != nil {
			return errors.Wrap(err, "could not write to postgresql.conf file")
		}

	}
	if err := writer.Flush(); err != nil {
		return errors.Wrap(err, "could not flush writer to postgresql.conf file")
	}
	return nil
}
...
func RunWithPostgres(m *testing.M, client **postgres.Client, fileLoader migrations.FileLoader, migrationDirectories ...string) {
	fn := func() int {
		...
		migrator.MustRunMigrations(context.Background(), fileLoader, migrationDirectories...)
