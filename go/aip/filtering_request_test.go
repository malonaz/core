...
func TestFilteringRequestParser_LogicalOperators(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedClause: "WHERE ((author.display_name = $1) AND (author.biography = $2))",
		...
		},
		{
			...
			expectedClause: "WHERE (((author.display_name = $1) AND (author.biography = $2)) AND (author.email_address = $3))",
			expectedParams: []any{"John", "Bio", "john@test.com"},
		},
		{
			name:           "OR with two conditions",
			filter:         `display_name = "John" OR display_name = "Jane"`,
			expectedClause: "WHERE ((author.display_name = $1) OR (author.display_name = $2))",
			expectedParams: []any{"John", "Jane"},
		},
		{
			name:           "OR with three conditions",
			filter:         `display_name = "A" OR display_name = "B" OR display_name = "C"`,
			expectedClause: "WHERE (((author.display_name = $1) OR (author.display_name = $2)) OR (author.display_name = $3))",
...
func TestFilteringRequestParser_OperatorPrecedence(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedClause: "WHERE ((author.display_name = $1) AND ((author.display_name = $2) OR (author.display_name = $3)))",
			expectedParams: []any{"A", "B", "C"},
		},
		{
		...
		},
		{
			...
			expectedClause: "WHERE (((author.display_name = $1) AND ((author.display_name = $2) OR (author.display_name = $3))) AND ((author.display_name = $4) OR (author.display_name = $5)))",
	...
	t.Run("Author metadata", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE (author.metadata->>'country' = $1)",
	...
	t.Run("Book metadata strings", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE (book.metadata->>'summary' = $1)",
				expectedParams: []any{"A great book"},
			},
			{
				name:           "nested language field",
				filter:         `metadata.language = "en"`,
				expectedClause: "WHERE (book.metadata->>'language' = $1)",
	...
	t.Run("Author fields", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				filter:         `display_name:*`,
				expectedClause: "WHERE (author.display_name IS NOT NULL AND author.display_name != '')",
				expectedParams: []any{},
			},
			{
				name:           "string field is not present",
				filter:         `NOT display_name:*`,
				expectedClause: "WHERE (NOT (author.display_name IS NOT NULL AND author.display_name != ''))",
				expectedParams: []any{},
			},
			{
				name:           "nested message field is present",
				filter:         `metadata:*`,
				expectedClause: "WHERE (author.metadata IS NOT NULL)",
				expectedParams: []any{},
			},
			{
				name:           "nested message field is not present",
				filter:         `-metadata:*`,
				expectedClause: "WHERE (NOT (author.metadata IS NOT NULL))",
				expectedParams: []any{},
			},
			{
			...
			},
			{
				...
				expectedClause: "WHERE (author.email_addresses IS NOT NULL AND COALESCE(array_length(author.email_addresses, 1), 0) > 0)",
				expectedParams: []any{},
			},
			{
				name:           "map contains key",
				filter:         `labels:"environment"`,
				expectedClause: "WHERE (COALESCE(author.labels, '{}') ? $1)",
				expectedParams: []any{"environment"},
			},
			{
				name:           "map does not contain key",
				filter:         `NOT labels:"deprecated"`,
				expectedClause: "WHERE (NOT (COALESCE(author.labels, '{}') ? $1))",
				expectedParams: []any{"deprecated"},
			},
			{
				name:           "map field is present",
				filter:         `labels:*`,
				expectedClause: "WHERE (author.labels IS NOT NULL AND author.labels != '{}'::jsonb)",
				expectedParams: []any{},
			},
			{
			...
			},
			{
				...
				expectedClause: "WHERE (($1 = ANY(author.email_addresses)) AND (author.display_name = $2))",
			...
			},
			{
				...
				expectedClause: "WHERE ((author.display_name IS NOT NULL AND author.display_name != '') AND (author.display_name = $1))",
				expectedParams: []any{"John"},
			},
			{
				name:           "timestamp field is present",
				filter:         `create_time:*`,
				expectedClause: "WHERE (author.create_time IS NOT NULL)",
				expectedParams: []any{},
			},
		}
...
func TestFilteringRequestParser_WildcardStringMatching(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedClause: "WHERE (author.display_name LIKE $1)",
			expectedParams: []any{"John%"},
		},
		{
		...
		},
		{
			...
			expectedClause: "WHERE (author.metadata->>'country' LIKE $1)",
		...
		},
		{
			...
			expectedClause: "WHERE ((author.display_name LIKE $1) AND (author.email_address = $2))",
			expectedParams: []any{"John%", "john@example.com"},
		},
		{
			name:           "wildcard with OR",
			filter:         `display_name = "John*" OR display_name = "Jane*"`,
			expectedClause: "WHERE ((author.display_name LIKE $1) OR (author.display_name LIKE $2))",
			expectedParams: []any{"John%", "Jane%"},
		},
	}
...
func TestFilteringRequestParser_Timestamps(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedClause: "WHERE (author.create_time = $1)",
		...
		},
		{
			...
			expectedClause: "WHERE (author.update_time <= $1)",
		...
		},
		{
			...
			expectedClause: "WHERE ((author.create_time >= $1) AND (author.create_time < $2))",
		...
		},
		{
			...
			filter:         `delete_time:*`,
			expectedClause: "WHERE (author.delete_time IS NOT NULL)",
			expectedParams: []any{},
		},
		{
...
func TestFilteringRequestParser_Enums(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedClause: "WHERE ((shelf.genre = $1) AND (shelf.display_name = $2))",
		...
		},
		{
			...
			expectedClause: "WHERE ((shelf.genre = $1) OR (shelf.genre = $2))",
	...
	t.Run("Author complex filters", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE (((author.display_name = $1) AND (author.email_address = $2)) AND (author.metadata->>'country' = $3))",
			...
			},
			{
				...
				expectedClause: "WHERE (((author.display_name IS NOT NULL AND author.display_name != '') AND (author.metadata IS NOT NULL)) AND (author.email_addresses IS NOT NULL AND COALESCE(array_length(author.email_addresses, 1), 0) > 0))",
				expectedParams: []any{},
			},
			{
				name:           "wildcard string with map check",
				filter:         `display_name = "John*" AND labels:"env"`,
				expectedClause: "WHERE ((author.display_name LIKE $1) AND (COALESCE(author.labels, '{}') ? $2))",
			...
			},
			{
				...
				expectedClause: "WHERE (((author.display_name = $1) OR (author.display_name = $2)) AND ((author.email_address = $3) OR (author.email_address = $4)))",
	...
	t.Run("Shelf complex filters with enum", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE (((shelf.genre = $1) AND (shelf.display_name = $2)) AND ((shelf.legacy_meta->>'capacity')::bigint > $3))",
			...
			},
			{
				...
				expectedClause: "WHERE (((shelf.genre = $1) OR (shelf.genre = $2)) AND ((shelf.legacy_meta->>'capacity')::bigint >= $3))",
	...
	t.Run("Book complex filters with integers", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				expectedClause: "WHERE ((book.metadata->>'language' = $1) AND (COALESCE(book.labels, '{}') ? $2))",
	...
	t.Run("allowed paths work", func(t *testing.T) {
		tests := []struct {
			...
			},
			{
				...
				expectedClause: "WHERE (COALESCE(book.labels, '{}') ? $1)",
	...
	t.Run("combined allowed fields", func(t *testing.T) {
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE (((book.title = $1) AND (book.publication_year >= $2)) AND (book.metadata->>'language' = $3))",
...
func TestFilteringRequestParser_ColumnNameReplacement(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedClause: "WHERE (((shelf.display_name = $1) AND (shelf.ext_id = $2)) AND ((shelf.legacy_meta->>'capacity')::bigint >= $3))",
	...
	t.Run("Shelf nullable duration", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				filter:         `duration:*`,
				expectedClause: "WHERE (shelf.duration IS NOT NULL)",
				expectedParams: []any{},
			},
			{
				name:           "duration not present",
				filter:         `NOT duration:*`,
				expectedClause: "WHERE (NOT (shelf.duration IS NOT NULL))",
				expectedParams: []any{},
			},
			{
			...
			},
			{
				...
				expectedClause: "WHERE ((shelf.duration >= $1) AND (shelf.duration < $2))",
				expectedParams: []any{10 * time.Second, time.Hour},
			},
			{
				name:           "duration combined with other fields",
				filter:         `duration:* AND display_name = "Test"`,
				expectedClause: "WHERE ((shelf.duration IS NOT NULL) AND (shelf.display_name = $1))",
	...
	t.Run("Book non-nullable duration", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				name:           "duration presence check",
				filter:         `duration:*`,
				expectedClause: "WHERE (book.duration IS NOT NULL)",
				expectedParams: []any{},
			},
			{
				name:           "duration equals 5s",
			...
			},
			{
				...
				expectedClause: "WHERE (((book.duration > $1) AND (book.title = $2)) AND (book.publication_year >= $3))",
	...
	t.Run("Book nested duration in metadata", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				expectedClause: "WHERE ((REPLACE(book.metadata->>'duration', 's', ''))::double precision IS NOT NULL)",
				expectedParams: []any{},
			},
			{
			...
			},
			{
				...
				expectedClause: "WHERE (((REPLACE(book.metadata->>'duration', 's', ''))::double precision >= $1) AND ((REPLACE(book.metadata->>'duration', 's', ''))::double precision < $2))",
			...
			},
			{
				...
				expectedClause: "WHERE (((REPLACE(book.metadata->>'duration', 's', ''))::double precision > $1) AND (book.title = $2))",
