...
// NewResourceUndeletedEvent constructs a ResourceUndeletedEvent by marshaling the given resource into an Any.
func NewResourceUndeletedEvent[R Resource](resource R) (*aippb.ResourceEvent, error) {
	resourceAny, err := anypb.New(resource)
	if err != nil {
		return nil, fmt.Errorf("marshaling %s to Any: %w", resource.ProtoReflect().Descriptor().FullName(), err)
	}
	return &aippb.ResourceEvent{
		Name:     resource.GetName(),
		Type:     aippb.ResourceEventType_RESOURCE_EVENT_TYPE_UNDELETED,
		Resource: resourceAny,
	}, nil
}
