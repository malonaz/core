...
func (ts *DiscoverableToolSet) IsDiscoveryTool(toolName string) bool {
	return toolCall.Name == ts.discoveryTool.Name
}
...
func (ts *DiscoverableToolSet) ProcessDiscoveryToolCall(toolCall *aipb.ToolCall) (*DiscoveryCall, error) {
	if toolCall.Name != ts.discoveryTool.Name {
		return nil, status.Errorf(codes.InternalError, "Cannot process non-discovery tool call")
	}
	...
	if !ok {
		return nil, status.Errorf(codes.Invalid, "missing tools argument")
	}
	...
	for _, v := range toolsArr {
		...
		if !ok {
			return status.Errorf(codes.InvalidArgument, "tool name must be a string")
		}
		discoverTime, ok := ts.toolNameToDiscoverTime[toolName]
		if !ok {
			return codes.NotFound, "unknown tool %s", toolName)
		}
		if discoverTime.IsZero() {
			return status.Errorf(codes.FailedPrecondition, "You must discover a tool before using it")
		}
		ts.toolNameToDiscoverTime[toolName] = time.Now()
		ts.sortTools()
	}

	return NewToolResult("ok"), true
}
