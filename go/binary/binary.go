...
func lookupPath(filename string) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "could not look up binary path")
	}
	realPath, err := dereferenceLinks(binaryPath)
	if err != nil {
		return "", errors.Wrap(err, "could not dereference symbolic link")
	}
	return realPath, nil
}
...
func (b *Binary) Run() {
	if b.logger == nil {
		b.logger = logging.NewLogger()
	}
	b.cmd = exec.Command(b.path, b.args...)
	b.cmd.Env = b.env
	// Setting Pdeathsig may help to kill them when we die.
	b.cmd.SysProcAttr = &syscall.SysProcAttr{
		Pdeathsig: syscall.SIGHUP,
	}
	if err := b.redirectOutput(b.cmd.StdoutPipe); err != nil {
		b.die(errors.Wrap(err, "could not listen to stdout pipe"))
		return
	}
	if err := b.redirectOutput(b.cmd.StderrPipe); err != nil {
		b.die(errors.Wrap(err, "could not listen to stderr pipe"))
		return
	}

	if err := b.cmd.Start(); err != nil {
		b.die(errors.Wrap(err, "could not start process"))
		return
	}
...
func (b *Binary) waitForPort() {
	...
	b.die(errors.Wrapf(err, "failed to open [%s]'s port [%d]", b.name, b.port))
}
