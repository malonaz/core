...
func JobName(job *model.Job) string {
	switch {
	case job.UserID != nil:
		return (&schedulerpb.OrganizationsUsersJobRn{Organization: *job.OrganizationID, User: *job.UserID, Job: job.JobID}).String()
	case job.OrganizationID != nil:
		return (&schedulerpb.OrganizationsJobRn{Organization: *job.OrganizationID, Job: job.JobID}).String()
	}
	return (&schedulerpb.JobRn{Job: job.JobID}).String()
}
