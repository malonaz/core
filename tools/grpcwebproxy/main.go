...
var (
	...
	shutdownGracePeriod = pflag.Duration(
		"shutdown_grace_period",
		30*time.Second, // Default value of 30 seconds
		"Duration to wait for graceful shutdown before force termination",
	)

	flagLogLevel = pflag.String("log_level", "warn", "Log level to set (panic, fatal, error, warn, info, debug, trace)")
)
...
func main() {
	...
	if err != nil {
		// If parsing fails, you may want to log the error and exit or default to a specific log level
		logrus.Fatalf("Invalid log level: %v", *flagLogLevel)
	}
	logrus.SetOutput(os.Stdout)

	// Create a logger for grpc at the error level.
	grpcLogger := logrus.StandardLogger()
	grpcLogger.SetLevel(level)
	grpcLogEntry := logrus.NewEntry(grpcLogger)

	logger := logrus.New()
	logger.SetLevel(logrus.InfoLevel)

	// Setup channel for interrupt signals to perform a graceful shutdown.
	...
	for _, flag := range pflag.Args() {
		if flag == "true" || flag == "false" {
			logger.Fatal("Boolean flags should be set using --flag=false, --flag=true or --flag (which is short for --flag=true). You cannot use --flag true or --flag false.")
		}
		logger.Fatal("Unknown argument: " + flag)
	}

	if *flagAllowAllOrigins && len(*flagAllowedOrigins) != 0 {
		logger.Fatal("Ambiguous --allow_all_origins and --allow_origins configuration. Either set --allow_all_origins=true OR specify one or more origins to whitelist with --allow_origins, not both.")
	}

	backendConn := dialBackendOrFail()

	grpcServer := buildGrpcProxyServer(backendConn, grpcLogEntry)
	...
	if *useWebsockets {
		logger.Println("using websockets")
		options = append(
			options,
		...
		if *websocketPingInterval >= time.Second {
			logger.Infof("websocket keepalive pinging enabled, the timeout interval is %s", websocketPingInterval.String())
		}
	...
	if !*runHttpServer && !*runTlsServer {
		logger.Fatalf("Both run_http_server and run_tls_server are set to false. At least one must be enabled for grpcweb proxy to function correctly.")
	}
	...
	if *enableHealthEndpoint {
		logger.Printf("health endpoint enabled on /%v", *healthEndpointName)
		if *enableHealthCheckService {
			logger.Printf("health checking enabled for service '%v'", *healthServiceName)
	...
	go func() {
		...
		logger.Infof(
			"Received shutdown signal, shutting down servers with grace period: %v",
			*shutdownGracePeriod,
		)
		ctx, cancel := context.WithTimeout(context.Background(), *shutdownGracePeriod)
		defer cancel()

		// Use a wait group to shutdown debug and tls server simulaneously.
		...
		if *runHttpServer {
			...
			go func() {
				...
				if err := debugServer.Shutdown(ctx); err != nil && !errors.Is(err, http.ErrServerClosed) {
					logger.Errorf("shutting down debug server: %v", err)
				}
				logger.Infof("http server graceful stop took %v", time.Since(start))
			}()
		}
		...
		if *runTlsServer {
			...
			go func() {
				defer wg.Done() // Decrement the WaitGroup counter when the goroutine completes
				if err := servingServer.Shutdown(ctx); err != nil && !errors.Is(err, http.ErrServerClosed) {
					logger.Errorf("shutting down tls server: %v", err)
				}
				logger.Infof("https server graceful stop took %v", time.Since(start))
			}()
		}

		wg.Wait() // Wait for both server shutdowns to complete

		// GracefulStop in gRPC allows any existing RPCs to complete without
		// accepting new ones. Note that this should be an instant one, given that we've shutdown
		// the debug and tls servers already.
		start = time.Now()
		grpcServer.GracefulStop()
		logger.Infof("grpc server graceful stop took %v", time.Since(start))

		// Signal that shutdown is complete.
		done <- true
	}()

	// Wait for server stop or a shutdown to complete.
	select {
	case err := <-errChan:
		logger.Warnf("gRPC web server stopped unexpectedly: %v", err)
	case <-done:
		logger.Info("gRPC web server stopped gracefully")
	}
