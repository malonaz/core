...
const (
	// decimalPkg backs google.type.Decimal fields in the model struct.
	// decimalProtoPkg holds the generated proto type. Both packages are named
...
func (m *Model) toPbFieldConversion(field *protogen.Field, fieldOpts *modelpb.FieldOpts, modelGoName string) (string, error) {
	...
	if protofield.IsDecimal(field) {
		fmt.Fprintf(&b, "\tvar %s *%s\n", goName, m.fqn(decimalProtoPkg, "Decimal"))
		if nullable {
			fmt.Fprintf(&b, "\tif m.%s != nil {\n", goName)
		}
		fmt.Fprintf(&b, "\t%s = &%s{Value: m.%s.String()}\n", goName, m.fqn(decimalProtoPkg, "Decimal"), goName)
		if nullable {
			fmt.Fprintf(&b, "\t}\n")
		}
		return b.String(), nil
