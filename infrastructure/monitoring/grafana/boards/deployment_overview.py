from grafanalib.core import (
    Row, Target, YAxis, YAxes, Tooltip, RED, GREEN, Dashboard, Templating,
)

from common.python.grafana.core import (
    template, print_dashboard, templating,
)
...
def information_row(
    ...
    cpu_params = format_params({
        "pod": "~([[deployment]]-[a-z0-9]+[-]?[a-z0-9]*|[deployment]]-[0-9]+)$",
        "container": "!",
        "resource": "cpu",
    })
    cpu_requests = Min(
    ...
    params = format_params({"pod": "~[[deployment]].*"})
    pod_status = Sum(
        vector=InstantVector(
