...
var (
	...
	ShelfWritePostgresColumns = postgres.GetDBColumns(model.Shelf{}, postgres.ExceptColumns("best_book_page_count", "latest_book", "latest_book_title", "latest_draft_book", "total_page_count", "book_count", "total_price", "last_book_create_time", "uninventoried_book_count", "oldest_uninventoried_book"))
)

var shelfJoinSubqueryExpr = `,(SELECT best_book.page_count FROM library.book AS best_book WHERE best_book.organization_id = shelf.organization_id AND best_book.shelf_id = shelf.shelf_id AND best_book.book_id = split_part(shelf.best_book, '/', 6)) AS best_book_page_count,(SELECT 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.page_count > 0) ORDER BY book.create_time DESC NULLS LAST, book.book_id LIMIT 1) AS latest_book,(SELECT book.title FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.page_count > 0) ORDER BY book.create_time DESC NULLS LAST, book.book_id LIMIT 1) AS latest_book_title,(SELECT 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.title LIKE 'Draft%') ORDER BY book.create_time DESC NULLS LAST, book.book_id LIMIT 1) AS latest_draft_book,(SELECT SUM(book.page_count) FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.page_count > 0)) AS total_page_count,(SELECT COUNT(*) FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS book_count,(SELECT SUM(book.price) FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS total_price,(SELECT MAX(book.create_time) FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS last_book_create_time,(SELECT COUNT(*) FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND ((NOT (shelf.inventory_time IS NOT NULL)) OR (book.create_time > shelf.inventory_time))) AS uninventoried_book_count,(SELECT 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND ((NOT (shelf.inventory_time IS NOT NULL)) OR (book.create_time > shelf.inventory_time)) ORDER BY book.create_time ASC NULLS LAST, book.book_id LIMIT 1) AS oldest_uninventoried_book`
var shelfJoinSelectExprs = `,best_book.page_count AS best_book_page_count,latest_book.name AS latest_book,latest_book.title AS latest_book_title,latest_draft_book.name AS latest_draft_book,total_page_count.value AS total_page_count,book_count.value AS book_count,total_price.value AS total_price,last_book_create_time.value AS last_book_create_time,uninventoried_book_count.value AS uninventoried_book_count,oldest_uninventoried_book.name AS oldest_uninventoried_book`
var shelfJoinClause = `LEFT JOIN library.book AS best_book ON best_book.organization_id = shelf.organization_id AND best_book.shelf_id = shelf.shelf_id AND best_book.book_id = split_part(shelf.best_book, '/', 6) LEFT JOIN LATERAL (SELECT book.*, 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id AS name FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.page_count > 0) ORDER BY book.create_time DESC NULLS LAST, book.book_id LIMIT 1) AS latest_book ON TRUE LEFT JOIN LATERAL (SELECT book.*, 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id AS name FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.title LIKE 'Draft%') ORDER BY book.create_time DESC NULLS LAST, book.book_id LIMIT 1) AS latest_draft_book ON TRUE LEFT JOIN LATERAL (SELECT SUM(book.page_count) AS value FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND (book.page_count > 0)) AS total_page_count ON TRUE LEFT JOIN LATERAL (SELECT COUNT(*) AS value FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS book_count ON TRUE LEFT JOIN LATERAL (SELECT SUM(book.price) AS value FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS total_price ON TRUE LEFT JOIN LATERAL (SELECT MAX(book.create_time) AS value FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id) AS last_book_create_time ON TRUE LEFT JOIN LATERAL (SELECT COUNT(*) AS value FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND ((NOT (shelf.inventory_time IS NOT NULL)) OR (book.create_time > shelf.inventory_time))) AS uninventoried_book_count ON TRUE LEFT JOIN LATERAL (SELECT book.*, 'organizations/' || book.organization_id || '/shelves/' || book.shelf_id || '/books/' || book.book_id AS name FROM library.book AS book WHERE book.organization_id = shelf.organization_id AND book.shelf_id = shelf.shelf_id AND ((NOT (shelf.inventory_time IS NOT NULL)) OR (book.create_time > shelf.inventory_time)) ORDER BY book.create_time ASC NULLS LAST, book.book_id LIMIT 1) AS oldest_uninventoried_book ON TRUE`
...
var (
	...
	ShelfWithRequestIDWritePostgresColumns = postgres.GetDBColumns(ShelfWithRequestID{}, postgres.ExceptColumns("best_book_page_count", "latest_book", "latest_book_title", "latest_draft_book", "total_page_count", "book_count", "total_price", "last_book_create_time", "uninventoried_book_count", "oldest_uninventoried_book"))
